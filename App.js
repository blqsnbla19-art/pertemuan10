...
// Kamus kode cuaca WMO (dari Open-Meteo) -> label & emoji
const WEATHER_CODES = {
...
export default function App() {
  // STATE: 4 variabel kelola UI
  const [searchInput, setSearchInput] = useState('');
  const [weatherData, setWeatherData] = useState(null);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState(null);

  // EFFECT: jalan tiap searchInput berubah
  useEffect(() => {
    // Input kosong -> reset tampilan
    if (!searchInput.trim()) {
      setWeatherData(null);
      setError(null);
      setLoading(false);
    ...
    const timeoutId = setTimeout(async () => {
      ...
      setError(null);
      try {
        // STEP 1: Geocoding (mengubah nama kota -> koordinat latitude & longitude)
        const geoUrl =
        ...
        // STEP 2: Forecast (mengubah koordinat -> data cuaca asli)
        const cuacaUrl =
        ...
        // Menyimpan hasil gabungan kedua API ke dalam state weatherData
        setWeatherData({
          kota: lokasi.name,
        ...
        // Mengabaikan error akibat pembatalan fetch (AbortError)
    ...
    // CLEANUP: Dipanggil jika user mengetik huruf baru sebelum 500ms tercapai
    return () => {
  ...
  return (
    <ScrollView style={styles.container} contentContainerStyle={styles.content}>
      ...
      {/* Kondisi 1: Menampilkan Loading Spinner */}
      {loading && (
      ...
      {/* Kondisi 3: Menampilkan Hasil Kartu Cuaca (Sukses) */}
      ...
      {/* Kondisi 4: Tampilan Awal saat Input Masih Kosong */}
      {!searchInput && !loading && (
        <View style={styles.center}>
